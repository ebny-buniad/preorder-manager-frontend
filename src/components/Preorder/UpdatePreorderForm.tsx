...
export default function UpdatePreorderForm({ data }: any) {
    const form = useForm({
        defaultValues: {
            name: data?.name ?? "",
            products: data?.products ?? 0,
            preorderWhen:
                data?.preorderWhen ?? "regardless-of-stock",
        ...
        onSubmit: async ({ value }) => {
            ...
            try {
                const res = await preordersService.updatePreorder(preorderData, data?.id)
